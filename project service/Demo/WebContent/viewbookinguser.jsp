...
String qr="select s.name,s.email as pemail,stype,charges,dt,bstatus from serviceprovider s,user u,booking where booking.pemail=s.email and booking.uemail=u.email and u.email=?";
...
tr
	...
	do
	{
		...
		<td><%=dt %></td>
		<td><%=status %></td>
		<%if(!(status.equals("complete")||status.equals("Accept"))){ %>
		<td><a href="DeleteRequest.jsp?uemail=<%=email %>&pemail=<%=pemail %>&dt=<%=dt %>">Delete</a></td>
		<%} %>
		<td><a href="CompleteRequest.jsp?uemail=<%=email %>&pemail=<%=pemail %>&dt=<%=dt %>">Complete</a></td>
		</tr>
		<%
	}while(rs.next());
	%>
